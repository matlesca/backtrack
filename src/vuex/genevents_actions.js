...
export function generateDateTags({ state, dispatch }) {
  return new Promise((resolve, reject) => {
    ...
    let monthCol = [
      460,
      460, // janvier février hiver
      254504, // mars desert
      13533, // avril flowers
      199320, // mai flowers
      256460, // juin fermes
      291441,
      291441, // juillet aout vacation
      332741, // sept school
      273263, // oct autumn
    ...
    for (let yy = startDate.year(); yy < endDate.year() + 1; yy++) {
      ...
      setTimeout(() => {
        if (
          getIntervalSongs({ state }, pushObj.start, pushObj.end, 0).length >= 5
        ) {
      ...
      seasons.forEach((season) => {
        if (
          startDate.isBefore(yy + "-" + season.end) &&
          endDate.isAfter(yy + "-" + season.start)
        ) {
          let pushObj = {
          ...
          if (yy === today.year()) {
            if (
              today.isAfter(yy + "-" + season.start) &&
              today.isBefore(yy + "-" + season.end)
            ) {
              pushObj.fromNow =
                state.locale === "fr"
                  ? "cet " + season.name[state.locale]
                  : "this " + season.name[state.locale];
              pushObj.featured = true;
            } else {
              pushObj.fromNow =
                season.name[state.locale] === "printemps"
                  ? "le printemps dernier"
                  : state.locale === "fr"
                  ? "l'" + season.name[state.locale] + " dernier"
                  : "last " + season.name[state.locale];
            }
          } else if (
            today.isBefore(moment(yy + "-" + season.end).add(1, "year"))
          ) {
            pushObj.fromNow =
              season.name[state.locale] === "printemps"
                ? "le printemps dernier"
          ...
          setTimeout(() => {
            if (
              getIntervalSongs({ state }, pushObj.start, pushObj.end, 0)
                .length >= 5
            ) {
              dispatch("ADD_DATECARD", pushObj);
            }
      ...
      for (let mm = 1; mm < 13; mm++) {
        ...
        if (
          startDate.isBefore(moment(yy + "-" + stmm + "-01").endOf("month")) &&
          endDate.isAfter(yy + "-" + stmm + "-01")
        ) {
          let pushObj = {
            type: state.locale === "fr" ? "mois" : "month",
            name: moment.months()[mm - 1] + " " + yy,
            start: yy + "-" + stmm + "-1",
            end: moment(yy + "-" + stmm + "-1")
              .endOf("month")
              .format("YYYY-MM-DD"),
            rank: 30,
          ...
          if (today.isSame(moment(yy + "-" + stmm + "-15"), "month")) {
            pushObj.fromNow =
              state.locale === "fr" ? "ce mois-ci" : "this month";
            pushObj.logo = "calendar";
          } else if (
            today.isSame(
              moment(yy + "-" + stmm + "-15").add(1, "month"),
              "month"
            )
          ) {
            pushObj.fromNow =
              state.locale === "fr" ? "le mois dernier" : "last month";
            pushObj.logo = "loop";
            pushObj.featured = true;
          } else {
            for (let gg = 2; gg < 7; gg++) {
              if (
                today.isSame(
                  moment(yy + "-" + stmm + "-15").add(gg, "month"),
                  "month"
                )
              ) {
                pushObj.fromNow =
    ...
    let specialDays = [
      ...
      },
      {
        ...
        m: "01",
        y: null,
        name: { en: "New year's Day", fr: "Jour de l'an" },
        logo: "fireworks",
        collection: 235549,
      },
      {
        ...
        m: "06",
        y: null,
        name: { en: "Summer solstice", fr: "Fête de la musique" },
        logo: "music",
        collection: 244173,
      },
      {
        ...
        m: "02",
        y: null,
        name: { en: "Valentine's Day", fr: "Saint Valentin" },
        logo: "love",
        collection: 208403,
      },
      {
        ...
        m: 10,
        y: null,
        name: { en: "Halloween", fr: "Halloween" },
        logo: "halloween",
        collection: 286795,
      },
      {
        ...
        m: 11,
        y: 2012,
        name: { en: "Thanksgiving", fr: "Thanksgiving" },
        logo: "chicken",
        collection: 285897,
      },
      {
      ...
      },
      {
        ...
        m: 11,
        y: 2012,
        name: { en: "Black friday", fr: "Black friday" },
        logo: "computer",
        collection: 145103,
      },
      {
      ...
      },
      {
        ...
        m: "01",
        y: 2012,
        name: { en: "Chinese new year", fr: "Nouvel an Chinois" },
        logo: "chinese",
        collection: 1248,
      },
      {
      ...
      specialDays.push(
        {
          ...
          m: "06",
          y: 2012,
          name: { en: "Mother's Day", fr: "Fête des mères" },
          logo: "family",
          collection: 233395,
        },
        {
        ...
        },
        {
          ...
          m: "07",
          y: null,
          name: { en: "Bastille day", fr: "Fête nationale Française" },
          logo: "fireworks",
          collection: 306296,
        }
      );
    ...
    // Father's day
    if (
      state.auth.country === "ES" ||
      state.auth.country === "IT" ||
      state.auth.country === "PT"
    ) {
      specialDays.push({
        d: 19,
        m: "03",
        y: null,
        name: { en: "Father's Day", fr: "Fête des pères" },
        logo: "family",
        collection: 233395,
      });
    } else {
    ...
    for (let cc = 0; cc <= diffDays; cc++) {
      ...
      if (today.isSame(dDate, "day")) {
        ...
        pushObj.fromNow =
          state.locale === "fr" ? "la semaine dernière" : "last week";
        pushObj.logo = "loop";
      } else if (today.isSame(dDate.clone().add(1, "month"), "day")) {
        pushObj.fromNow =
          state.locale === "fr"
            ? "même date le mois dernier"
      ...
      for (let ss = 0; ss < specialDays.length; ss++) {
        if (
          parseInt(specialDays[ss].d, 10) === dDate.date() &&
          parseInt(specialDays[ss].m, 10) === dDate.month() + 1 &&
          (!specialDays[ss].y || specialDays[ss].y === dDate.year())
        ) {
          pushObj.specialDay =
            specialDays[ss].name[state.locale] + " " + dDate.year();
      ...
      setTimeout(() => {
        if (
          getIntervalSongs({ state }, pushObj.start, pushObj.end, 0).length > 0
        ) {
          dispatch("ADD_DATECARD", pushObj);
        }
      }, delayTimer);
