...
export function initApp({ dispatch }, http) {
  return new Promise(function (resolve, reject) {
    ...
    // })
    var r = Math.round(
      40 + Math.round(Math.random()) * 40 + Math.random() * 40
    );
    var g = Math.round(
      40 + Math.round(Math.random()) * 40 + Math.random() * 40
    );
      ...
      function (response) {
        if (response.authResponse) {
          ...
          reject({
            type: "auth",
            message: {
              en: "Couldn't connect, please try to log-in again",
              fr: "Erreur de connexion à votre compte",
            },
          });
        }
      },
...
export function getAllSongs({ dispatch, state }) {
  return new Promise(function (resolve, reject) {
    if (state.histoBound === 0) {
      reject({
        message: "please setup the histo bound before running getAllSongs",
      });
    } else {
    ...
    if (index > 3000 && false) {
      ...
      setTimeout(() => {
        if (
          ...
          reject({
            message: "The index & limit should be integers",
            index: index,
            limit: limit,
          });
        }
  ...
  getAPISongsHisto(state.histoBound, 40, true, state.delayRequests)
    .then((result) => {
      ...
      if (result.data.length >= 40) {
        ...
        if (state.histoBound > 10) {
          resolve(result.data);
        } else {
          reject({
            type: "histo",
            message: {
              en: "Your listening history is too short, try again in a few days !",
              fr: "Pas assez d'historique d'écoute, réessaye dans quelques jours !",
            },
          });
        }
      }
    ...
    .catch((error) => {
      if (error.code === 200) {
        ...
        reject({
          type: "perm",
          message: {
            en: "Permission error, please try to log-in again",
            fr: "Autorisations insuffisantes, réessayer svp",
          },
        });
      } else if (error.code === 800) {
        // No data found, try with a lower bound
        dispatch("INC_TIMESFAILED");
        if (state.timesFailed > 100) {
          reject({
            type: "histo",
            message: {
              en: "Your listening history is too short, try again in a few days !",
        ...
        if (state.timesFailed > 10) {
          reject({
            type: "server",
            message: {
              en: "No answer from server, please try again later",
              fr: "Pas de réponse du serveur, réessayer plus tard svp",
            },
          });
        } else {
          dispatch("INC_TIMESFAILED");
...
function loopDailySongsHisto(dispatch, state, resolve, reject, indexesTab) {
  ...
  if (indexesTab) {
    indexesTab.forEach((ind) => {
      proms.push(getAPISongsHisto(ind, 40, false, state.delayRequests));
    });
  } else {
    for (var pp = 0; pp < Math.ceil(state.histoBound / 40); pp++) {
      proms.push(getAPISongsHisto(40 * pp, 40, false, state.delayRequests));
    }
  }
          ...
          currentProm
            .then((result) => {
              ...
              setTimeout(() => {
                ...
                if (result.index === 0) {
                  dispatch("SET_DATEBOUNDS", {
                    last: moment
                      .unix(result.data[0].timestamp)
                      .format("YYYY-MM-DD"),
                  });
                }
                dispatch("SET_DATEBOUNDS", {
                  first: moment
                    .unix(result.data[result.data.length - 1].timestamp)
                    .format("YYYY-MM-DD"),
                });
                resProm();
              }, 0);
  ...
  allProms
    .then(() => {
      if (missedIndexes.length) {
        ...
        if (state.timesFailed > 10) {
          reject({
            message: "Failed 10 times in a row, try again later please.",
            code: 4,
          });
        } else {
        ...
        dispatch(
          "INC_LOADINGSONGSINDEX",
          state.loadingSongsIndex - state.histoBound - 1
        );
        resolve();
      }
    })
